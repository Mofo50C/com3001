...
void hashmap_entry_init(hashmap_entry_t *e, uintptr_t key, uint64_t hash, uint64_t value)
{
	e->key = key;
	e->hash = hash;
	e->occupied = 1;
...
int _hashmap_insert(hashmap_t *h, uintptr_t key, uint64_t hash, uint64_t value, int update)
{
...
void hashmap_resize(hashmap_t *h)
{
	...
	for (i = 0; i < old_cap; i++) {
		...
		_hashmap_insert(h, entry->key, entry->hash, entry->index, 0);
	}
	...
	do {
		...
		if (!found && entry->key == key) {
			...
			(slot < prev_slot && (actual_slot <= prev_slot && actual_slot > slot))) {
				hashmap_entry_init(&h->table[prev_slot], entry->key, entry->hash, entry->index);
				hashmap_entry_clear(entry);
				prev_slot = slot;
			}
		}
